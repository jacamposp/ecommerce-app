...
export async function AccountButton() {
  ...
  return (
    <form action={signOutAction} className="fixed top-6 right-24 z-50">
      <button
        type="submit"
        className="flex size-12 items-center justify-center overflow-hidden rounded-full border border-white/15 bg-black/40 text-white backdrop-blur-md transition-all duration-200 hover:border-white/30 hover:bg-black/60"
        aria-label={`Sign out, signed in as ${session.user.name}`}
